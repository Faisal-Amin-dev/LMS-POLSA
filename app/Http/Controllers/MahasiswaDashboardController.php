...
{
    // 1. BERANDA (Menampilkan Kartu Kelas Aktif)
    public function index()
    {
        ...
        // Ambil kelas aktif yang diikuti mahasiswa ini
        $classrooms = $mahasiswa->classrooms()->where('status', 'aktif')->with(['course', 'dosen'])->latest()->get();

        return view('mahasiswa.dashboard', compact('mahasiswa', 'classrooms'));
    }

// 2. DETAIL RUANG KELAS (Tab Beranda, Materi, Tugas, Anggota)
    public function show(Request $request, $id)
    {
        $user = Auth::user();
        $mahasiswa = Mahasiswa::where('user_id', $user->id)->first();

        $kelas = $mahasiswa->classrooms()->where('classrooms.id', $id)->with(['course', 'dosen'])->firstOrFail();
